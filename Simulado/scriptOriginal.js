document.addEventListener('DOMContentLoaded', function() {
        ...
        if (checkboxes.length > 0) {
            checkboxes.forEach(checkbox => {
                checkbox.addEventListener('change', function() {
                    ...
                    qntRespostasMultiplas = respostasSelecionadas.length;
                    ...
                    const respostaCorreta = () => {
                        ...
                        statusQuestao.push({ questao: indice, respondido: true, isMultiple: true });
            ...
            radios.forEach(radio => {
                radio.addEventListener('change', function() {
                    ...
                    statusQuestao.push({ questao: indice, respondido: true, isMultiple: false });
    ...
    document.getElementById('btnProxima').addEventListener('click', function() {
        if (qntRespostas == 0 && statusQuestao.find(x => x.questao == indiceAtual)?.respondido == true) {}
        else{

            console.log("isMultiple", statusQuestao.find(x => x.questao == indiceAtual)?.isMultiple);
            
            if (!statusQuestao.find(x => x.questao == indiceAtual)?.isMultiple && qntRespostas < 1 ) {
                alert("Marque uma opção!")
                return
            }

            if ((statusQuestao.find(x => x.questao == indiceAtual)?.isMultiple) && qntRespostas < 2 ) {
                alert("Marque duas opções!")
                return
            }
        }
